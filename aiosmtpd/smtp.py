...
class SMTP(asyncio.StreamReaderProtocol):
    ...
    def eof_received(self):
        ...
        if self.session.ssl is not None:
            # If STARTTLS was issued, return False, because True has no
            # effect on an SSL transport and raises a warning. Our superclass
            # has no way of knowing we switched to SSL so it might return True.
            return False
